...
import { c as createComponent, a as createAstro, r as renderTemplate, m as maybeRenderHead, s as spreadAttributes, f as renderSlot, e as renderComponent, b as addAttribute, F as Fragment } from '../../chunks/astro/server_Cc2yo0Cj.mjs';
import 'kleur/colors';
...
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, {}, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<main> <article class="prose mx-auto"> ${post && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": ($$result3) => renderTemplate` <div class="prose-h1 text-center"> <h1>${post.title}</h1> </div> ${post.mainImage && renderTemplate`<div> <img class="h-[30%] w-full"${addAttribute(`${post.mainImage}`, "src")}${addAttribute(post.title, "alt")}> </div>`}<div class="mb-3 mt-4 flex justify-between">  </div> ${post.tags && post.tags.map((tag) => renderTemplate`<span class="m-1 gap-5 rounded-full bg-blue-400 p-2 text-white"> ${tag} </span>`)}<div> ${renderComponent($$result3, "PortableText", $$PortableText, { "portableText": post.body })} </div> ` })}`} </article> </main> ` })}`;
